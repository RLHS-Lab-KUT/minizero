...
- **案B を推奨** [推論]．Task B の目的（候補外の手を特定する）を最小の変更で満たし，
  0-2 で確かめたとおり root 直下の `count_ == 0` は -2 の手と1対1に対応する．
- GUI への影響 [要確認]：`xrl_viz/index.html` は未調査．案B・案C とも
  「子が増える／フィールドが増える」だけなので壊れないはずだが，**実際に動かして確認していない**．
- 案A は探索木そのものの見え方が変わるため，Phase II の説明設計を決めてからの方が安全 [推論]．
...
局面5 の「第一印象 -0.373 / 読んだあとの結論 +0.952」について，
旧 n128 記録の ply 58 の root は `v = 0.166 / Q = 0.470` で一致しない．
`v = -0.3729` に一致するのは **ply 59** の root．`+0.952` はどの ply とも一致しない
（ply 59 の Q は 0.976）．1 ply ずれた値を引いている可能性があるが，確認できていない．

### 実現可能な検証経路（**未実行**．実行の可否を確認したい）

旧 n128 の ply 0〜58 の着手列を，各 n のエンジンに `reg_genmove` + `play` で打ち直す．
`replay.py` と同じ方式で，参照する棋譜を n16 から旧 n128 に差し替えるだけ．
...
- **案1**：上記の打ち直しを実行し，「修正後のエンジンがこの局面をどう判断するか」を得る．
  局面の性質（終盤・一目と読みの乖離）が保たれているかを確認できる．
- **案2**：修正後の n=128 記録から，同じ性質（k が小さい終盤・v と Q が大きく乖離）の局面を
  選び直して局面5 を差し替える．
- **案3**：局面5 を落として5局面にする．
...
1. Step 0-4 の修正案（A/B/C）の選択．**適用していない．**
2. Step 3 の検証経路（案1/2/3）の選択．**実行していない．**
3. §2.3 の [要確認] を外すかの判断（docs は書き換えていない）．
4. 局面5 の補助数値の出所 [要確認]．
5. `xrl_viz/index.html` が新フィールド・子ノード増加に耐えるかの実動確認 [要確認]．
